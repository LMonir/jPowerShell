 ...
 * For doing that, you can either call manually close() or implement a try with
 * resources as it implements {@link AutoCloseable}.
 *
...
public class PowerShell implements AutoCloseable {
	...
	// Default PowerShell termination string
	private static final String DEFAULT_WIN_TASKKILL = "taskkill.exe /PID %d /F /T";
	private static final String DEFAULT_LINUX_TASKKILL = "kill -9 %d";
	...
	public PowerShell configuration(Map<String, String> config) {
		try {
			...
			this.tempFolder = (config != null && config.get("tempFolder") != null)
					? getTempFolder(config.get("tempFolder"))
	 ...
	 * @throws PowerShellNotAvailableException if PowerShell is not installed in the
	 *                                         system
	 */
	 ...
	 * Creates a session in PowerShell console an returns an instance which allows
	 * to execute commands in PowerShell context.<br>
	 * This method allows to define a PowersShell executable path different from
	 * default
	 *
	 * @param customPowerShellExecutablePath the path of powershell executable. If
	 *                                       you are using the default installation
	 *                                       path, call {@link #openSession()}
	 *                                       method instead
	 * @return an instance of the class
	 * @throws PowerShellNotAvailableException if PowerShell is not installed in the
	 *                                         system
	...
	public static PowerShell openSession(String customPowerShellExecutablePath) throws PowerShellNotAvailableException {
		final PowerShell powerShell = new PowerShell();
	...
	private PowerShell initalize(String powerShellExecutablePath) throws PowerShellNotAvailableException {
		...
		// Init thread pool. 2 threads are needed: one to write and read console and the
		// other to close it
